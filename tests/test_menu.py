...
def test_view_available_items(client: TestClient, db_session: Session):
    ...
    for i in range(5):
        food = Food(
            name=f'Available {i}', price=10.0, category=category, is_available=True
        )
        db_session.add(food)
    for i in range(2):
        food = Food(
            name=f'Unavailable {i}', price=10.0, category=category, is_available=False
        )
        db_session.add(food)
    db_session.commit()
...
def test_category_filtering(client: TestClient, db_session: Session):
    ...
    foods_category = Category(
        name='Foods', description='Food category', display_order=2
    )
    ...
    for i in range(2):
        food = Food(
            name=f'Food {i}', price=15.0, category=foods_category, is_available=True
        )
        db_session.add(food)
    db_session.commit()
...
def test_fetch_categories(client: TestClient, db_session: Session):
    ...
    for index, cat in enumerate(categories, start=1):
        category = Category(
            name=cat, description=f'{cat} category', display_order=index
        )
        db_session.add(category)
    db_session.commit()
...
def test_menu_pagination(client: TestClient, db_session: Session):
    ...
    for i in range(15):
        food = Food(
            name=f'Food {i:02d}', price=10.0, category=category, is_available=True
        )
        db_session.add(food)
    db_session.commit()
