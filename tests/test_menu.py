...
def test_view_available_items(client: TestClient, db_session: Session):
    ...
    for i in range(5):
        food = Food(name=f"Available {i}", price=10.0, category="Test", is_available=True)
        db_session.add(food)
    for i in range(2):
        food = Food(name=f"Unavailable {i}", price=10.0, category="Test", is_available=False)
        db_session.add(food)
    db_session.commit()
...
def test_category_filtering(client: TestClient, db_session: Session):
    ...
    for i in range(3):
        ...
        db_session.add(food)
    for i in range(2):
        food = Food(name=f"Food {i}", price=15.0, category="Foods", is_available=True)
        db_session.add(food)
    db_session.commit()

    response = client.get("/api/v1/foods/?category=Drinks")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 3
    for item in data:
...
def test_fetch_categories(client: TestClient, db_session: Session):
    ...
    for cat in categories:
        food = Food(name=f"Item {cat}", price=10.0, category=cat, is_available=True)
        db_session.add(food)
    db_session.commit()
...
def test_menu_pagination(client: TestClient, db_session: Session):
    ...
    # Setup: 15 items
    for i in range(15):
        food = Food(name=f"Food {i:02d}", price=10.0, category="Test", is_available=True)
        db_session.add(food)
    db_session.commit()
