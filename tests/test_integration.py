...
class TestIntegration:
    ...
    def test_guest_order_flow(
        self, client, db_session, guest_user, sample_table, sample_food
    ):
        """
    ...
    def test_registered_user_order_flow(
        self, client, auth_headers, db_session, sample_table, sample_food
    ):
    ...
    def test_large_dataset_performance(self, db_session):
        ...
        try:
            service.calculate_retention_rate(days=30)
            duration = time.time() - start
            assert (
                duration < 1.0
            )  # Should be fast for small dataset, scalable for large
