...
class TestTableStatus:
    ...
    def test_table_session_status_occupied(
        self, client, db_session, sample_table, sample_food, authenticated_user
    ):
...
class TestRetentionAnalytics:
    ...
    def test_retention_rate_calculation(
        self, client, db_session, admin_headers, create_users
    ):
        ...
        new_order = Order(
            user_id=user.id, table_id=1, status=OrderStatus.PENDING, created_at=now
        )
    ...
    def test_customer_segments_demographics(
        self, client, db_session, admin_headers, create_users
    ):
    ...
    def test_inventory_alerts_high_priority(
        self, client, db_session, admin_headers, sample_food
    ):
        ...
        response = client.get(
            '/api/v1/analytics/inventory-alerts?hours=24', headers=admin_headers
        )
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
