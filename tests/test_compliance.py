...
class TestGDPRCompliance:
    ...
    def test_self_data_export_success(
        self, client, db_session, authenticated_user, auth_headers
    ):
        """TC-GDPR-01: Self Data Export - Authenticated User A exports A"""
        response = client.get(
            f'/api/v1/gdpr/{authenticated_user.id}/export', headers=auth_headers
        )
    ...
    def test_unauthorized_data_export(
        self, client, db_session, create_users, auth_headers
    ):
...
class TestSecurityCompliance:
    ...
    def test_sensitive_history_endpoint_removed(
        self, client, authenticated_user, auth_headers
    ):
        """TC-SEC-01: Sensitive History - GET /orders/user/{uid} must be 404/405"""
        response = client.get(
            f'/api/v1/orders/user/{authenticated_user.id}', headers=auth_headers
        )
