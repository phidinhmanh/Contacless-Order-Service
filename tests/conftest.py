...
def create_tables(db_session: Session) -> List[Table]:
    ...
    tables = []
    for i in range(1, 6):
        table = Table(table_number=i, capacity=4 if i <= 3 else 6, is_occupied=False)
...
def create_users(db_session: Session) -> dict:
    ...
    with patch(
        'app.core.security.get_password_hash', side_effect=lambda p: f'hashed_{p}'
    ):
...
def auth_token(authenticated_user: User) -> str:
    ...
    return create_access_token(
        subject=str(authenticated_user.id), expires_delta=timedelta(hours=24)
    )
...
def admin_token(admin_user: User) -> str:
    ...
    return create_access_token(
        subject=str(admin_user.id), expires_delta=timedelta(hours=24)
    )
...
@pytest.fixture
def create_payments(db_session: Session, create_orders: List[Order]) -> List[Payment]:
...
@pytest.fixture
def sample_payment(db_session: Session, sample_order: Order) -> Payment:
...
@pytest.fixture
def low_stock_food(db_session: Session, create_menu: List[Food]) -> Food:
...
def expired_token() -> str:
    return create_access_token(subject='999', expires_delta=timedelta(minutes=-30))
...
def seed_full_database(
    create_tables, create_menu, create_users, create_orders, create_payments
) -> dict:
