...
class TestOrderCancellationStockRestoration:
    ...
    def test_tc_cancel_stock_02_admin_delete_restores_quantity(
        self, client, db_session: Session, admin_headers, auth_headers, sample_table, sample_food
    ):
    ...
    def test_tc_cancel_stock_03_multiple_items_all_restored(
        ...
        # Cancel order
        cancel_response = client.post(f"/api/v1/orders/{order_id}/cancel", headers=auth_headers)
        assert cancel_response.status_code == status.HTTP_200_OK
    ...
    def test_tc_cancel_stock_04_cancel_after_window_no_restore(
        self, client, db_session: Session, auth_headers, authenticated_user, sample_table, sample_food
    ):
        ...
        cancel_response = client.post(f"/api/v1/orders/{order.id}/cancel", headers=auth_headers)
    ...
    def test_tc_cancel_stock_05_double_cancel_no_double_restore(
        ...
        second_cancel = client.post(f"/api/v1/orders/{order_id}/cancel", headers=auth_headers)
    ...
    def test_tc_cancel_stock_09_different_user_cannot_cancel(
        ...
        user1_token = create_access_token(subject=str(user1.id), expires_delta=timedelta(hours=1))
        ...
        create_response = client.post("/api/v1/orders/", json=order_data, headers=user1_headers)
        order_id = create_response.json()["id"]

        # User 2 tries to cancel
        user2 = create_users["registered"][1]
        user2_token = create_access_token(subject=str(user2.id), expires_delta=timedelta(hours=1))
        user2_headers = {"Authorization": f"Bearer {user2_token}"}

        cancel_response = client.post(f"/api/v1/orders/{order_id}/cancel", headers=user2_headers)
        assert cancel_response.status_code == status.HTTP_403_FORBIDDEN

    def test_tc_cancel_stock_10_full_workflow_integration(
        self, client, db_session: Session, auth_headers, admin_headers, sample_table, create_menu
    ):
        ...
        for food in foods:
            db_session.refresh(food)
        assert foods[0].stock_quantity == 100, f"Expected 100, got {foods[0].stock_quantity}"
        assert foods[1].stock_quantity == 110, f"Expected 110, got {foods[1].stock_quantity}"
        assert foods[2].stock_quantity == 120, f"Expected 120, got {foods[2].stock_quantity}"
...
class TestAdminCancellationStockRestoration:
    ...
    def test_admin_delete_old_order_still_restores_stock(
        self, client, db_session: Session, admin_headers, authenticated_user, sample_table, sample_food
    ):
        ...
        # Admin deletes
        delete_response = client.delete(f"/api/v1/orders/{order.id}", headers=admin_headers)
        assert delete_response.status_code == status.HTTP_200_OK
