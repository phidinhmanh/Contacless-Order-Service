...
with engine.connect() as conn:
    ...
    for name, desc, order in categories:
        exists = conn.execute(text("SELECT id FROM categories WHERE name = :name"), {"name": name}).fetchone()
        if not exists:
            conn.execute(
                text("INSERT INTO categories (name, description, display_order) VALUES (:name, :desc, :order)"),
