...
from app.schemas.auth import GuestLoginRequest, RefreshTokenRequest, Token
...
def refresh_token(
    ...
    """
    payload = verify_token(token_request.refresh_token, token_type='refresh')  # nosec
...
def login_as_guest(
    ...
    if guest_user_id:
        user = (
            db.query(User)
            .filter(User.id == int(guest_user_id), User.phone_number.is_(None))
            .first()
        )
...
def update_guest_demographics(
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail='Guest user not found'
        )
