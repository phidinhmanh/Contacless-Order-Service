...
def get_orders(
    ...
    """Get orders for the current user (for customers) or all orders (for staff)."""

    # For customers (both guest and registered), only return their own orders
    if current_user.role == 'customer':
        ...
        # For staff/admin users, return all orders
...
def get_orders_by_status(
    ...
    """Get all orders by status. Accepts single status or comma-separated list."""
    if status is None:
        return []
...
def get_my_orders(
    ...
    """Get all orders for the current user (guest or registered) with pagination."""
...
def get_order(
    ...
    """Get a specific order by ID. Regular and guest users can only access their own orders."""
    order = crud_order.get(db, id=order_id)
    if not order:
        raise HTTPException(status_code=404, detail='Order not found')

    # Check if user has permission to access this order
    # Both guest and registered customers can only access their own orders
    # Staff, managers, and admins can access any order
    if current_user.role == 'customer':
...
async def pay_cash(
    ...
    payment_in = PaymentCreate(
        order_id=order_id, provider=PaymentProvider.CASH, amount=order.total_price
    )
