...
from app.schemas.table_session import TableSessionCreate, TableSessionResponse
...
def resolve_qr_token(qr_token: str, db: Session = Depends(get_db)):
    ...
    table = (
        db.query(Table)
        .filter(Table.qr_token == qr_token, Table.deleted_at.is_(None))
        .first()
    )
...
def get_table_session_status(table_id: int, db: Session = Depends(get_db)):
    ...
    active_orders = (
        db.query(Order)
        .filter(Order.table_id == table_id, Order.status.in_(active_statuses))
        .first()
    )
...
def create_table(
    ...
    background_tasks.add_task(
        manager.broadcast_to_all, {'type': 'tables_update', 'action': 'create'}
    )
    return table
...
def update_table(
    ...
    if table_in.table_number and table_in.table_number != old_number:
        try:
            ...
            qr_path = generate_qr_code(
                updated_table.id, updated_table.table_number, updated_table.qr_token
            )
    ...
    background_tasks.add_task(
        manager.broadcast_to_all, {'type': 'tables_update', 'action': 'update'}
    )
...
def delete_table(
    ...
    background_tasks.add_task(
        manager.broadcast_to_all, {'type': 'tables_update', 'action': 'delete'}
    )
...
def rotate_table_token(
    ...
    if table.qr_code_path:
        try:
            ...
            logger.error(f'Failed to delete QR code file: {e}')
