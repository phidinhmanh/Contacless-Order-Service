...
class AnalyticsService:
    ...
    def get_revenue_summary(
        ...
        # Handle potential None values from SQLAlchemy
        rev: float = 0.0
        if res.revenue is not None:
            rev = float(res.revenue)
    ...
    def get_customer_segments(self) -> Dict[str, Any]:
        ...
        if res is None:
            return {
                ...
                'demographics': {
                    ...
                    'age_groups': {
                        'under_18': 0, '18_24': 0, '25_34': 0, '35_44': 0, '45+': 0
                    },
                },
            }
        ...
        return {
            ...
            'retention_rate': round((returning / total_customers * 100), 2)
    ...
    def get_retention_data(self) -> Dict[str, Any]:
        ...
        def _retention(window_days: int) -> tuple[int, int, float]:
            ...
            rate = (
                round((returning_users / eligible_users * 100), 2)
                if eligible_users
                else 0
            )
    ...
    def get_peak_hours(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        peak_query = (
            self.db.query(
                func.to_char(func.timezone(self.tz, Order.created_at), 'HH24').label(
                    'hour'
                ),
                case(
                    (
                        func.extract('minute', func.timezone(self.tz, Order.created_at))
                        < 30,
                        '00',
                    ),
                    else_='30',
    ...
    def get_inventory_alerts(self, threshold_hours: int = 24) -> List[Dict[str, Any]]:
        ...
        velocity_sub = (
            ...
            .join(Order)
            .filter(Order.created_at >= limit)
            .group_by(OrderItem.food_id)
            .subquery()
        )

        foods = (
            self.db.query(Food.name, Food.stock_quantity, velocity_sub.c.units_per_hour)
            .outerjoin(velocity_sub, Food.id == velocity_sub.c.food_id)
            .all()
        )
        ...
        return sorted(
            alerts,
            key=lambda x: (x['priority'] == 'MEDIUM', x['estimated_hours_remaining']),
        )
        ...
        return [
            {
                ...
                'avg_order_value': float(r.total_revenue / r.order_count)
                if r.order_count > 0
                else 0,
            }
            for r in result
