...
class PaymentService:
    ...
    async def process_casso_webhook(self, payload: CassoWebhookPayload) -> list[Payment]:
        """
        ...
        if payload.error != 0:
            raise HTTPException(
                status_code=400,
                detail=f"Casso webhook error: {payload.error}"
            )
        ...
        for transaction in payload.data:
            ...
            if order.user:
                 ...
                 if sender_name and (not order.user.full_name or "Guest" in order.user.full_name):
